...
export function OrganigramaPage() {
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold text-neutral-900 text-neutral-800 mb-1">Organigrama</h1>
      <p className="text-sm text-neutral-500 text-neutral-400 mb-6">Estructura organizacional de Taty's Enterprises</p>
            ...
            {depto.equipos?.map((equipo: any) => (
              <div key={equipo.id} className="bg-neutral-50 border border-neutral-200 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-medium text-sm text-neutral-900 text-neutral-800">{equipo.nombre}</h3>
                  {equipo.supervisor && (
                    <Link to={`/equipo/${equipo.supervisor.id}`} className="text-xs text-primary-600 text-primary-600 hover:underline">
                ...
                <div className="divide-y divide-neutral-100 divide-neutral-200">
                  {equipo.miembros?.map((m: any) => (
                    <Link key={m.usuarioId} to={`/equipo/${m.usuarioId}`} className="flex items-center justify-between py-2 hover:bg-neutral-50 hover:bg-neutral-100 -mx-2 px-2 rounded">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-neutral-700 text-neutral-300">{m.nombre}</span>
                      </div>
                      <span className="text-xs text-neutral-500 text-neutral-400">{m.cargo}</span>
                    </Link>
                  ))}
                  {(!equipo.miembros || equipo.miembros.length === 0) && (
                    <p className="text-xs text-neutral-400 text-neutral-500 py-2">Sin miembros todavía.</p>
                  )}
                </div>
              </div>
            ))}
