...
export function PersonaDetailPage() {
  ...
  return (
    <div>
      <Link to="/personas" className="text-xs text-neutral-500 text-neutral-400 hover:text-primary-600 hover:text-primary-600">← Personas</Link>

      <div className="flex items-start justify-between mt-2 mb-4">
        <div>
          <h1 className="text-xl font-semibold text-neutral-900 text-neutral-800">{ficha.nombre}</h1>
          ...
          {ficha.fechaNacimiento && (
            <p className="text-xs text-pink-600 text-pink-400 mt-0.5">
          ...
          <div className="flex gap-1 mt-1.5 flex-wrap">
            {ficha.tags.map((t) => (
              <span key={t} className="bg-warning-50 bg-warning-500/15 text-warning-700 text-warning-600 text-[11px] px-2 py-0.5 rounded-full">{t}</span>
            ))}
            {ficha.negocios.map((n) => (
              <span key={n} className="bg-success-50 bg-success-500/15 text-success-700 text-success-600 text-[11px] px-2 py-0.5 rounded-full">{n}</span>
            ))}
          </div>
        </div>
        {puedeEditar && (
          <button onClick={abrirEdicion} className="text-xs border border-neutral-200 text-neutral-300 rounded-lg px-3 py-1.5 hover:bg-neutral-100 shrink-0">
            ✏️ Editar datos
          </button>
        )}
      </div>
        ...
        <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-4 mb-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={editTelefono} onChange={(e) => setEditTelefono(e.target.value)} className="w-full border border-neutral-200 bg-neutral-50 text-neutral-200 rounded-lg px-3 py-1.5 text-sm" />
            </div>
            <div>
              <label className="text-xs text-neutral-600 text-neutral-400">Email</label>
              <input value={editEmail} onChange={(e) => setEditEmail(e.target.value)} type="email" className="w-full border border-neutral-200 bg-neutral-50 text-neutral-200 rounded-lg px-3 py-1.5 text-sm" />
            </div>
            <div>
              <label className="text-xs text-neutral-600 text-neutral-400">Ciudad *</label>
              <input value={editCiudad} onChange={(e) => setEditCiudad(e.target.value)} required list="ciudades-editar" className="w-full border border-neutral-200 bg-neutral-50 text-neutral-200 rounded-lg px-3 py-1.5 text-sm" />
            ...
            <div>
              ...
              */}
              <select value={editEstado} onChange={(e) => setEditEstado(e.target.value)} required className="w-full border border-neutral-200 bg-neutral-50 text-neutral-200 rounded-lg px-3 py-1.5 text-sm">
            ...
            <div>
              ...
              <input value={editFechaNacimiento} onChange={(e) => setEditFechaNacimiento(e.target.value)} type="date" className="w-full border border-neutral-200 bg-neutral-50 text-neutral-200 rounded-lg px-3 py-1.5 text-sm" />
            </div>
          </div>
          {editError && <p className="text-[11px] text-danger-600 mt-2">{editError}</p>}
          <div className="flex items-center gap-3 mt-3">
            <button onClick={guardarDatos} disabled={guardandoDatos} className="text-xs bg-primary-500 text-white px-3 py-1.5 rounded-lg font-medium disabled:opacity-50">
              {guardandoDatos ? "Guardando..." : "Guardar"}
            </button>
            <button onClick={() => setEditando(false)} className="text-xs border border-neutral-200 text-neutral-300 rounded-lg px-3 py-1.5">
              Cancelar
            </button>
      ...
      <div className="flex gap-1 border-b border-neutral-200 border-neutral-200 mb-4">
        {TABS.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-3 py-2 text-sm border-b-2 -mb-px ${
              tab === t ? "border-primary-500 text-primary-600 text-primary-600 font-medium" : "border-transparent text-neutral-500 text-neutral-400"
            }`}
          >
            {t}
          </button>
          ...
          <div className="mt-5 flex flex-col gap-3">
            {ficha.timeline.map((ev, i) => (
              <div key={i} className="flex gap-3 text-sm">
                ...
                <div>
                  <p className="text-neutral-800 text-neutral-300">{ev.detalle}</p>
                  <p className="text-[11px] text-neutral-400 text-neutral-500">{fmtFecha(ev.fecha)}</p>
                </div>
              </div>
            ))}
            {ficha.timeline.length === 0 && <p className="text-sm text-neutral-400 text-neutral-500">Sin actividad todavía.</p>}
          </div>
        </div>
      )}
      ...
      {tab === "Comercial" && (
        <div>
          <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-3 mb-4">
            <p className="text-xs text-neutral-600 text-neutral-400 mb-2">Negocios de interés</p>
                ...
                <button
                  ...
                  className={`text-xs px-2.5 py-1 rounded-full border ${
                    negociosSeleccionados.includes(negocio)
                      ? "bg-success-50 bg-success-500/15 border-success-500 border-success-500/20 text-success-700 text-success-600"
          ...
          <div className="bg-neutral-50 bg-neutral-100 border border-neutral-200 rounded-lg p-3 mb-4 flex items-end gap-2">
            <div className="flex-1">
              <label className="text-xs text-neutral-600 text-neutral-400 block mb-1">Agregar a pipeline</label>
              <select
                value={pipelineElegido}
                onChange={(e) => setPipelineElegido(e.target.value)}
                className="w-full border border-neutral-200 bg-transparent text-neutral-200 rounded-lg px-2 py-1.5 text-sm"
              >
                {pipelines.map((p) => (
            ...
            <div className="w-32">
              <label className="text-xs text-neutral-600 text-neutral-400 block mb-1">Valor (opcional)</label>
              <input
                value={valorRegistro}
            ...
            <button
              ...
              className="text-xs bg-primary-500 text-white px-3 py-2 rounded-lg font-medium disabled:opacity-50"
            >
          ...
          <div className="bg-neutral-50 border border-neutral-200 rounded-xl divide-y divide-neutral-100 divide-neutral-200">
            {ficha.registros.length === 0 && <p className="p-4 text-sm text-neutral-400 text-neutral-500">Sin registros en ningún pipeline todavía.</p>}
            {ficha.registros.map((r) => (
              <div key={r.id} className="p-4 flex justify-between text-sm">
                <div>
                  <p className="font-medium text-neutral-900 text-neutral-800">{r.pipelineNombre}</p>
                  <p className="text-neutral-500 text-neutral-400 text-xs">{r.etapaNombre}</p>
                </div>
                {r.valor != null && <p className="font-medium text-neutral-900 text-neutral-800">${r.valor.toLocaleString()}</p>}
              </div>
            ))}
          </div>
        </div>
      ...
      {tab === "Comentarios" && (
        <div>
          <p className="text-xs text-neutral-500 text-neutral-400 mb-2">
            Notas generales visibles para todo el equipo — preferencias, contexto, lo que sea útil.
            {!esSuperAdmin && <span className="block mt-0.5 text-neutral-400 text-neutral-500">Solo un Super Admin puede editarlas.</span>}
          </p>
          {esSuperAdmin ? (
            <>
              <textarea
            ...
            <div className="w-full border border-neutral-200 rounded-lg px-3 py-2 text-sm bg-neutral-50 bg-neutral-100 text-neutral-700 text-neutral-300 min-h-[120px] whitespace-pre-wrap">
              {comentarios || <span className="text-neutral-400 text-neutral-500">Sin comentarios todavía.</span>}
            </div>
          )}
        </div>
      )}
      ...
      {tab === "Historial" && (
        <div className="flex flex-col gap-2">
          {ficha.historial.length === 0 && <p className="text-sm text-neutral-400 text-neutral-500">Sin historial todavía.</p>}
          {ficha.historial.map((h) => (
            <div key={h.id} className="p-3 rounded-lg border border-neutral-200 bg-neutral-50 text-sm">
              <p className="text-neutral-800 text-neutral-300">{h.accion}</p>
              <p className="text-[11px] text-neutral-400 text-neutral-500 mt-0.5">{h.autorNombre} · {fmtFecha(h.fecha)}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {tab === "Tareas" && (
        <div className="flex flex-col gap-2">
          {ficha.tareas.length === 0 && <p className="text-sm text-neutral-400 text-neutral-500">Sin tareas de seguimiento.</p>}
          {ficha.tareas.map((t) => (
            <div key={t.id} className={`flex items-center justify-between p-3 rounded-lg border ${t.completado ? "bg-neutral-50 bg-neutral-100 border-neutral-100 border-neutral-200 opacity-60" : "bg-neutral-50 border-neutral-200 border-neutral-200"}`}>
              <div>
                <p className="text-sm text-neutral-800 text-neutral-300">{t.nota}</p>
                <p className="text-xs text-neutral-400 text-neutral-500">{fmtFecha(t.fecha)}</p>
              </div>
              {!t.completado && (
