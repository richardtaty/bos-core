...
export function CommandCenterPage() {
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold text-neutral-900 mb-1">💵 Revenue Command Center</h1>
      <p className="text-sm text-neutral-500 mb-6">¿Dónde está el dinero y qué hago hoy para moverlo?</p>

      {/* Ask BOS */}
      <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-4 mb-6">
        <h2 className="text-sm font-medium text-neutral-700 mb-2">💬 Pregúntale a BOS</h2>
        <div className="flex gap-2 mb-2">
          <input
            ...
            className="flex-1 border border-neutral-200 bg-transparent text-neutral-800 rounded-lg px-3 py-2 text-sm"
          />
          <button
            onClick={() => void preguntar(pregunta)}
            disabled={preguntando || !pregunta.trim()}
            className="text-sm font-medium bg-primary-500 text-white px-4 py-2 rounded-lg disabled:bg-primary-100 disabled:text-primary-800"
          >
        ...
        <div className="flex flex-wrap gap-1.5">
          {[
            ...
            <button
              ...
              className="text-[11px] px-2.5 py-1 rounded-full border border-neutral-200 text-neutral-500 hover:bg-neutral-100"
            >
              {s}
            </button>
          ))}
        ...
        {respuesta && (
          <div className="mt-3 bg-neutral-100 border border-neutral-200 rounded-lg p-3">
            <p className="text-sm text-neutral-800 whitespace-pre-line">{respuesta}</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-warning-50 bg-warning-500/10 border border-warning-200 border-warning-500/20 rounded-xl p-5 mb-4">
        <p className="text-xs font-medium text-warning-700 mb-2">💰 CASH GAP</p>
        <div className="flex items-center gap-4 flex-wrap">
          <div>
            <p className="text-xs text-warning-700/70">Vendido</p>
            <p className="text-2xl font-semibold text-warning-800">{fmt(resumen.vendido)}</p>
          </div>
          <span className="text-warning-700 text-xl">−</span>
          <div>
            <p className="text-xs text-warning-700/70">Cobrado</p>
            <p className="text-2xl font-semibold text-warning-800">{fmt(resumen.cobrado)}</p>
          </div>
          <span className="text-warning-700 text-xl">=</span>
          <div>
            <p className="text-xs text-warning-700/70">Cash Gap</p>
            <p className="text-3xl font-bold text-warning-800">{fmt(resumen.cashGap)}</p>
          </div>
        </div>
        <p className="text-xs text-warning-700/70 mt-2">
          {resumen.cashGap > 0
      ...
      {data.alertas.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-medium text-neutral-700 mb-2">🚨 Alertas</h2>
            ...
            {data.alertas.map((a, i) => {
              ...
              return (
                <div key={i} className={`flex items-center justify-between gap-3 p-3 rounded-lg border ${color}`}>
                  <div>
                    <p className="text-sm font-medium text-neutral-900">{a.titulo}</p>
                    <p className="text-xs text-neutral-500">{a.detalle}</p>
                  </div>
      ...
      <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-5 mb-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold text-neutral-900">💰 Oportunidades de dinero hoy</h2>
          <p className="text-2xl font-bold text-success-600">{fmt(dineroHoy)}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5 text-sm">
          <div className="bg-danger-50 bg-danger-500/10 border border-danger-200 border-danger-500/20 rounded-lg p-3">
            <p className="text-xs text-danger-700">Cobros vencidos ({moneyToday.cobrosVencidos.length})</p>
            <p className="text-lg font-semibold text-danger-700">{fmt(resumen.vencido)}</p>
          </div>
          <div className="bg-primary-50 bg-primary-500/10 border border-primary-200 border-primary-500/20 rounded-lg p-3">
            <p className="text-xs text-primary-700">Pagos programados ({moneyToday.pagosProgramados.length})</p>
            <p className="text-lg font-semibold text-primary-700">{fmt(resumen.proximos7dias)}</p>
          </div>
          <div className="bg-warning-50 bg-warning-500/10 border border-warning-200 border-warning-500/20 rounded-lg p-3">
            <p className="text-xs text-warning-700">Ofertas calientes ({moneyToday.ofertasCalientes.length})</p>
            <p className="text-lg font-semibold text-warning-700">{fmt(resumen.pipelineCaliente)}</p>
          </div>
        </div>
        ...
        {topAcciones.length > 0 && (
          <div>
            <h3 className="text-sm font-medium text-neutral-700 mb-2">Top 5 acciones recomendadas</h3>
            <div className="flex flex-col gap-2">
              {topAcciones.map((a, i) => (
                <Link
                  key={i}
                  to="/pipelines"
                  className="flex items-center justify-between p-3 rounded-lg border border-neutral-200 bg-neutral-100 hover:opacity-80"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-neutral-500">{TIPO_LABEL[a.tipo]}{a.pipelineNombre ? ` · ${a.pipelineNombre}` : ""}</p>
                    </div>
                  </div>
      ...
      {oportunidades.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-medium text-neutral-700 mb-2">🔥 Ranking de oportunidades</h2>
          <div className="flex flex-col gap-2">
            {oportunidades.slice(0, 10).map((o, i) => {
              const scoreColor =
                o.score >= 70 ? "text-success-600" : o.score >= 40 ? "text-warning-600" : "text-neutral-500";
              return (
                <Link
                  key={i}
                  to="/pipelines"
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-neutral-900 truncate">{o.personaNombre ?? "Sin nombre"}</p>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-[11px] text-neutral-500">{o.valor != null ? fmt(o.valor) : "—"} · {o.accion}</p>
                  </div>
                </Link>
              );
            })}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <h3 className="text-sm font-medium text-neutral-700 mb-2">🔴 Cobros vencidos</h3>
          {moneyToday.cobrosVencidos.length === 0 ? (
            <p className="text-xs text-neutral-500">Nada vencido. 👌</p>
          ) : (
            <div className="flex flex-col gap-2">
              {moneyToday.cobrosVencidos.map((c) => (
                <div key={c.registroId} className="flex justify-between p-3 rounded-lg border border-danger-100 border-danger-500/20 bg-danger-50 bg-danger-500/10 text-sm">
                  <div>
                    <p className="font-medium text-neutral-900">{c.personaNombre}</p>
                    <p className="text-[11px] text-neutral-500">{c.pipelineNombre} · vence {fmtFecha(c.fecha)}</p>
                  </div>
                  <p className="font-semibold text-danger-600">{fmt(c.monto)}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <div>
          <h3 className="text-sm font-medium text-neutral-700 mb-2">📅 Próximos pagos (7 días)</h3>
          {moneyToday.pagosProgramados.length === 0 ? (
            <p className="text-xs text-neutral-500">Sin pagos programados próximamente.</p>
          ) : (
            <div className="flex flex-col gap-2">
              {moneyToday.pagosProgramados.map((p) => (
                <div key={p.registroId} className="flex justify-between p-3 rounded-lg border border-neutral-200 bg-neutral-100 text-sm">
                  <div>
                    <p className="font-medium text-neutral-900">{p.personaNombre}</p>
                    <p className="text-[11px] text-neutral-500">{p.pipelineNombre} · {fmtFecha(p.fecha)}</p>
                  </div>
                  <p className="font-semibold text-primary-600">{fmt(p.monto)}</p>
                </div>
              ))}
            </div>
          )}
