...
exports.reportePodcast = reportePodcast;
...
async function reportePodcast() {
    ...
    if (!pipeline)
        throw new Error("El pipeline de Podcast no existe todavía");
    const etapasPipeline = await client_1.db.select().from(schema_1.etapas).where((0, drizzle_orm_1.eq)(schema_1.etapas.pipelineId, pipeline.id)).orderBy(schema_1.etapas.orden);
    const registrosPipeline = await client_1.db.select().from(schema_1.registros).where((0, drizzle_orm_1.eq)(schema_1.registros.pipelineId, pipeline.id));
    const etapaPorId = new Map(etapasPipeline.map((e) => [e.id, e]));
    const ordenDe = (nombre) => etapasPipeline.find((e) => e.nombre === nombre)?.orden ?? -1;
    ...
    const llegoA = (ordenObjetivo) => registrosPipeline.filter((r) => {
        const e = etapaPorId.get(r.etapaId);
        if (!e)
            return false;
        if (e.orden === noShowOrden)
            return false; // no-show nunca "llegó" a nada más adelante
    ...
    if (registroIds.length > 0) {
        for (const id of registroIds) {
            const [fila] = await client_1.db.select({ total: (0, drizzle_orm_1.sum)(schema_1.pagos.monto) }).from(schema_1.pagos).where((0, drizzle_orm_1.eq)(schema_1.pagos.registroId, id));
