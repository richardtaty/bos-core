...
exports.redactarEmail = redactarEmail;
exports.resumenUnderwriting = resumenUnderwriting;
...
async function completar(sistema, usuario) {
    if (!(0, funding_env_1.iaActiva)())
        return null;
    let respuesta;
    try {
        respuesta = await fetch("https://api.anthropic.com/v1/messages", {
            method: "POST",
            headers: {
                "x-api-key": funding_env_1.ANTHROPIC_API_KEY,
                "anthropic-version": "2023-06-01",
                "content-type": "application/json",
            },
            body: JSON.stringify({
                model: funding_env_1.ANTHROPIC_MODELO,
                max_tokens: 600,
                system: sistema,
...
async function redactarEmail(solicitud, proposito) {
    const contexto = `Application ${solicitud.applicationId} — ${solicitud.empresaLegal ?? "business"}\n` +
...
async function resumenUnderwriting(solicitud) {
    const contexto = "Summarize this funding application for an internal underwriting review. " +
