...
import { ESTADOS_USA, esEstadoValido } from "../lib/estados-usa";
import { actualizarDatosPersona } from "../services/personas.service";
 ...
 */
const DECISIONES_MANUALES: Record<string, string> = {
...
type Fila = { id: string; nombre: string; ciudad: string; estado: string };
type Propuesta = {
  fila: Fila;
  valor: string;
  via: string;
};
/** Una lectura del texto: qué estado sugiere y qué palabra la disparó. */
type Lectura = { valor: string; patron: string };
 ...
 */
function buscarMasLargo(txt: string, tabla: [string, string][]): Lectura | null {
  const ordenada = [...tabla].sort((a, b) => b[0].length - a[0].length);
  for (const [patron, valor] of ordenada) {
    if (txt.includes(norm(patron))) return { valor, patron };
  }
  return null;
}

const TABLA_ESTADOS: [string, string][] = [
  ...Object.entries(ALIAS),
  ...ESTADOS_USA.filter((e) => e !== "Fuera de USA").map((e) => [e, e] as [string, string]),
];
const TABLA_CIUDADES: [string, string][] = Object.entries(CIUDADES);
const TABLA_FUERA: [string, string][] = FUERA.map((f) => [f, "Fuera de USA"]);

const porNombreDeEstado = (txt: string) => buscarMasLargo(txt, TABLA_ESTADOS);
const porCiudad = (txt: string) => buscarMasLargo(txt, TABLA_CIUDADES);
const porFuera = (txt: string) => buscarMasLargo(txt, TABLA_FUERA);

function porSigla(txt: string): Lectura | null {
  const palabras = txt.split(" ");
  for (const s of SIGLAS_SEGURAS) {
    if (palabras.includes(s.toLowerCase())) return { valor: SIGLAS[s], patron: s };
  }
  return null;
}
...
async function main() {
  ...
  const filas = (await db
    ...
    .from(personas)) as Fila[];

  const yaCorrectos: Fila[] = [];
  const propuestas: Propuesta[] = [];
  const sinDeterminar: Fila[] = [];
  ...
  for (const fila of filas) {
    ...
    const canonico = ESTADOS_USA.find((e) => e.toLowerCase() === actual.toLowerCase());
    if (canonico) {
    ...
    const lecturas = [nombre, porCiudad(txt), porSigla(txt)].filter(
      (v): v is Lectura => v !== null
    );
  ...
  const invalidas = propuestas.filter((p) => !esEstadoValido(p.valor));
  if (invalidas.length > 0) {
    console.error(`\n✗ ${invalidas.length} propuestas tienen un estado inválido. Abortando.`);
    for (const p of invalidas) console.error(`   ${p.fila.nombre}: "${p.valor}"`);
    process.exit(1);
  }
  ...
  for (const p of propuestas) porVia.set(p.via, (porVia.get(p.via) ?? 0) + 1);
  ...
  // Aplicar
  // -------------------------------------------------------------------------
  const [autor] = await db.select().from(usuarios).where(eq(usuarios.rol, "SUPER_ADMIN"));
  if (!autor) {
  ...
  for (const p of propuestas) {
    try {
      await actualizarDatosPersona(p.fila.id, { estado: p.valor }, { id: autor.id, rol: autor.rol });
      ok++;
    } catch (e) {
      fallos.push(`${p.fila.nombre}: ${(e as Error).message}`);
    }
  }
