...
import { crearSolicitudFundingSchema } from "../lib/validation";
import { crearSolicitudFunding } from "../services/bmf-solicitudes.service";
import { db } from "../db/client";
import { bmfSolicitudes, bmfMensajes } from "../db/schema";
import { BMF_CHAT_URL, BMF_CHAT_KEY, bmfChatActivo } from "../lib/funding-env";
...
export const publicBmfRouter = Router();
...
const _rangoPorIp = new Map<string, number[]>();
...
function ipDe(req: { ip?: string; headers: Record<string, string | string[] | undefined> }): string {
...
publicBmfRouter.post("/funding-aplicar", async (req, res) => {
  try {
    const ip = ipDe(req);
    if (!permitido(ip)) {
    ...
    const parsed = crearSolicitudFundingSchema.safeParse(req.body);
    if (!parsed.success) {
      res.status(400).json({ error: parsed.error.flatten() });
      return;
    }

    const resultado = await crearSolicitudFunding(parsed.data);
...
const _chatPorIp = new Map<string, number[]>();

function chatPermitido(ip: string): boolean {
  const ahora = Date.now();
...
publicBmfRouter.post("/bmf-chat", async (req, res) => {
  try {
    const ip = ipDe(req);
    if (!chatPermitido(ip)) {
    ...
    const cuerpo = (req.body ?? {}) as { messages?: { role?: string; content?: unknown }[] };
    ...
    try {
      respuesta = await fetch(`${BMF_CHAT_URL}/v1/chat/completions`, {
        method: "POST",
        signal: controlador.signal,
        headers: {
          Authorization: `Bearer ${BMF_CHAT_KEY}`,
...
function extraerEmail(v: unknown): string | null {
  if (typeof v === "string") {
    const m = v.match(/<([^>]+)>/);
  ...
  if (v && typeof v === "object") {
    const obj = v as { email?: unknown };
    if (typeof obj.email === "string" && obj.email.includes("@")) return obj.email.trim();
  }
  return null;
}

publicBmfRouter.post("/resend-inbound", async (req, res) => {
  try {
    const evento = (req.body ?? {}) as { data?: Record<string, unknown> };
    ...
    const [solicitud] = await db
      .select({ id: bmfSolicitudes.id, personaId: bmfSolicitudes.personaId })
      .from(bmfSolicitudes)
      .where(eq(bmfSolicitudes.propietarioEmail, emailRemitente))
